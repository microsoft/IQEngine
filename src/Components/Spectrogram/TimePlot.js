...
function randomFrequencyPoints() {
    var pts = new Array(1000);
    var count = 0
    for (var i = 0; i < 1000; i += .01) {
        pts[count] =  Math.sin(i/.05) + 1;
        count += 1;
    }
    return pts;
}
            ...
            <Modal isOpen={modal} toggle={toggle} size="lg">
                ...
                <ModalBody>
                    <div style={{ marginBottom:'15px' }}>
                        <input type="radio" value="time" onChange={onChangeSelectedOption} checked={selectedOption === 'time'} style={{ marginRight:'5px' }}/>Time
                        <input type="radio" value="frequency" onChange={onChangeSelectedOption} checked={selectedOption === 'frequency'} style={{ marginLeft:'15px', marginRight:'5px' }} />Frequency
                    </div>
                    { selectedOption === 'time' && 
                    <Plot 
                        data={[{
